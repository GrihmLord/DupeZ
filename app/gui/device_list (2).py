...
class DeviceList(QWidget):
    ...
    def create_device_item(self, device) -> QListWidgetItem:
        ...
        try:
            ...
            status_icon = "[BLOCKED]" if device.blocked else "[ACTIVE]"
    ...
    def toggle_data_encryption(self):
        ...
        try:
            if self.encrypt_data_btn.isChecked():
                ...
                for i in range(self.device_list.count()):
                    item = self.device_list.item(i)
                    device_data = item.data(Qt.ItemDataRole.UserRole + 1)
                    if device_data:
                        # Encrypt sensitive fields
                        if device_data.get('mac') and device_data['mac'] != "Unknown":
                            encrypted_mac = self.cipher.encrypt(device_data['mac'].encode())
                            device_data['mac'] = base64.b64encode(encrypted_mac).decode()
                        
                        if device_data.get('hostname') and device_data['hostname'] != "Unknown":
                            encrypted_hostname = self.cipher.encrypt(device_data['hostname'].encode())
                            device_data['hostname'] = base64.b64encode(encrypted_hostname).decode()
                        
                        # Update item data
                        item.setData(Qt.ItemDataRole.UserRole + 1, device_data)
                        self.update_device_item_display(item, device_data)
                
                log_info("Device data encrypted successfully")
            else:
                ...
                if hasattr(self, 'cipher'):
                    for i in range(self.device_list.count()):
                        ...
                        if device_data:
                            ...
                            try:
                                if device_data.get('mac') and device_data['mac'] != "Unknown":
                                    encrypted_mac = base64.b64decode(device_data['mac'].encode())
                                    decrypted_mac = self.cipher.decrypt(encrypted_mac).decode()
                                    device_data['mac'] = decrypted_mac
                                
                                if device_data.get('hostname') and device_data['hostname'] != "Unknown":
                                    encrypted_hostname = base64.b64decode(device_data['hostname'].encode())
                                    decrypted_hostname = self.cipher.decrypt(encrypted_hostname).decode()
                                    device_data['hostname'] = decrypted_hostname
                                
                                # Update item data
                                item.setData(Qt.ItemDataRole.UserRole + 1, device_data)
                                self.update_device_item_display(item, device_data)
                            except Exception as e:
                                log_error(f"Error decrypting data: {e}")
                    
                    log_info("Device data decrypted successfully")
        except Exception as e:
    ...
    def toggle_selected_devices_unblocking(self):
        ...
        try:
            ...
            if unblocked_count > 0:
                self.status_label.setText(f"[SUCCESS] UNBLOCKED {unblocked_count} DEVICES")
