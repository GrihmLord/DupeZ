...
class NetworkNode(QObject):
    ...
    def __init__(self, device_id: str, ip_address: str, mac_address: str, 
                 hostname: str, device_type: str, status: str):
        super().__init__()
        ...
        self.status = status  # online, offline, blocked, suspicious
        self.x: float = 0.0
        self.y: float = 0.0
        self.traffic_in: int = 0
        self.traffic_out: int = 0
...
class NetworkTopologyWidget(QWidget):
    ...
    def update_topology(self):
        """Update the topology with real network data"""
        try:
            # Import real network scanner
            from app.network.enhanced_scanner import get_enhanced_scanner
            from app.core.traffic_analyzer import get_traffic_analyzer
            ...
            traffic_data = traffic_analyzer.get_current_traffic()
            
            # Update existing nodes with real traffic data
            for device_id, node in self.topology_view.scene.nodes.items():
                # Get real traffic data for this device
                device_traffic = traffic_data.get(node.ip_address, {})
                traffic_in = device_traffic.get('bytes_in', 0)
                traffic_out = device_traffic.get('bytes_out', 0)
                
                self.topology_view.scene.update_node_traffic(device_id, traffic_in, traffic_out)
            
            # Add real discovered devices
            for device in devices:
                if device.get('ip') and device.get('mac'):
                    # Check if device already exists
                    device_exists = any(
                        node.ip_address == device['ip'] 
                        for node in self.topology_view.scene.nodes.values()
                    )
                    
                    if not device_exists:
                        self.add_real_device(device)
            
        except Exception as e:
            log_error(f"Error updating topology: {e}")
    
    def add_real_device(self, device_data: dict):
        """Add a real discovered device to the topology"""
        try:
            # Determine device type based on real data
            device_type = self._determine_device_type(device_data)
            
            # Determine status based on real network state
            status = self._determine_device_status(device_data)
            
            node = NetworkNode(
                device_id=device_data.get('mac', device_data.get('ip', '')),
            ...
            log_info(f"Added real device to topology: {node.hostname} ({node.ip_address})")
            
        except Exception as e:
            log_error(f"Error adding real device: {e}")
    
    def _determine_device_type(self, device_data: dict) -> str:
        """Determine device type based on real network data"""
        try:
            vendor = device_data.get('vendor', '').lower()
            hostname = device_data.get('hostname', '').lower()
            
            # PS5 detection
            if 'sony' in vendor or 'ps5' in hostname or 'playstation' in hostname:
                return 'ps5'
            
            # Router detection
            if any(router in vendor for router in ['tp-link', 'netgear', 'asus', 'linksys', 'd-link']):
                return 'router'
            
            # Gaming console detection
            if any(console in hostname for console in ['xbox', 'nintendo', 'switch']):
                return 'gaming'
            
            # Mobile device detection
            if any(mobile in vendor for mobile in ['apple', 'samsung', 'huawei', 'xiaomi']):
                return 'mobile'
            ...
            log_error(f"Error determining device type: {e}")
            return 'unknown'
    
    def _determine_device_status(self, device_data: dict) -> str:
        """Determine device status based on real network state"""
        try:
            response_time = device_data.get('response_time', 0)
            last_seen = device_data.get('last_seen', '')
            
            # Check if device is currently online
            if response_time > 0 and response_time < 5:  # Good response time
                return 'online'
            ...
            log_error(f"Error determining device status: {e}")
            return 'unknown'
