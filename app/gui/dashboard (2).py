...
class DupeZDashboard(QMainWindow):
    ...
    def setup_ui(self):
        """Setup the main user interface with enhanced responsive design"""
        self.setWindowTitle("DupeZ - Advanced LagSwitch Tool")
        self.setWindowIcon(QIcon("app/assets/icon.ico"))
        
        # Import responsive layout manager (temporarily disabled for testing)
        # from app.gui.responsive_layout_manager import ResponsiveLayoutManager, ResponsiveTabWidget, get_screen_info
        
        # Initialize responsive layout manager (temporarily disabled)
        # self.layout_manager = ResponsiveLayoutManager()
        
        # Get screen information (temporarily disabled)
        # screen_info = get_screen_info()
        # screen_width = screen_info['width']
        # screen_height = screen_info['height']
        
        # Use default screen size for now
        screen_width = 1366
        screen_height = 768
        
        # Calculate responsive window size based on screen size
        if screen_width <= 1024:  # Small screen
            window_width = int(screen_width * 0.95)  # Use more screen space
            window_height = int(screen_height * 0.95)
        elif screen_width <= 1366:  # Medium screen
            window_width = int(screen_width * 0.9)
            window_height = int(screen_height * 0.9)
        elif screen_width <= 1920:  # Large screen
            window_width = int(screen_width * 0.85)
            window_height = int(screen_height * 0.85)
        else:  # Ultra/4K screen
        ...
        # Main layout with responsive spacing (temporarily disabled)
        layout = QHBoxLayout()
        # spacing = self.layout_manager.get_responsive_spacing(8)
        ...
        if screen_width <= 1024:  # Small screen - compact sidebar
            sidebar_width = max(200, min(250, int(window_width * 0.18)))
        elif screen_width <= 1366:  # Medium screen
            sidebar_width = max(220, min(280, int(window_width * 0.20)))
        else:  # Large screen - comfortable sidebar
