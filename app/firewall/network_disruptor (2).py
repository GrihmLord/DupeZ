...
class NetworkDisruptor:
    ...
    def _get_mac_address(self, ip: str) -> str:
        """Get MAC address for IP using real network discovery"""
        try:
            ...
            # Check ARP table again after ping
            if platform.system().lower() == "windows":
                result = subprocess.run(
                    ['arp', '-a', ip], capture_output=True, text=True, timeout=5
                )
            ...
            try:
                result = subprocess.run(
                    ['nmap', '-sn', ip], capture_output=True, text=True, timeout=10
                )
                if result.returncode == 0:
            ...
            log_warning(f"Could not determine MAC address for {ip}, using broadcast MAC")
