#!/usr/bin/env python3
"""
Enhanced Network Scanner for PulseDrop Pro
...
class EnhancedNetworkScanner(QObject):
    ...
    def _get_traffic_info(self, ip: str) -> Tuple[int, int]:
        """Get traffic information for a device using real network monitoring"""
        try:
            # Use psutil to get network statistics
            net_io = psutil.net_io_counters(pernic=True)
            
            # Find the interface that contains this IP
            ...
            if target_interface and target_interface in net_io:
                ...
                stats = net_io[target_interface]
                
                # Return traffic levels (packets sent/received)
                return stats.packets_sent, stats.packets_recv
            else:
                # Fallback: use overall network stats
                overall_stats = psutil.net_io_counters()
                return overall_stats.packets_sent, overall_stats.packets_recv
                
        except Exception as e:
            log_error(f"Error getting traffic info for {ip}: {e}")
            return 0, 0
...
# Global scanner instance with better lifecycle management
_enhanced_scanner = None
_scanner_lock = threading.Lock()

def get_enhanced_scanner() -> EnhancedNetworkScanner:
    """Get or create enhanced scanner instance with thread safety"""
    ...
    with _scanner_lock:
        if _enhanced_scanner is None:
            try:
                ...
                log_info("Enhanced scanner instance created successfully")
            except Exception as e:
                log_error(f"Failed to create enhanced scanner: {e}")
                return None
        else:
            # Check if the scanner object is still valid
            try:
                # Try to access a simple attribute to check if object is still valid
                _ = _enhanced_scanner.max_threads
            except Exception:
                # Object was deleted, create a new one
                log_info("Scanner object was deleted, creating new instance")
                try:
                    _enhanced_scanner = EnhancedNetworkScanner()
                    log_info("New enhanced scanner instance created successfully")
                except Exception as e:
                    log_error(f"Failed to create new enhanced scanner: {e}")
                    return None
    
    return _enhanced_scanner

def cleanup_enhanced_scanner():
    """Clean up enhanced scanner with thread safety"""
    global _enhanced_scanner
    ...
    with _scanner_lock:
        if _enhanced_scanner:
            try:
                ...
                if _enhanced_scanner.scan_in_progress:
                    _enhanced_scanner.stop_scan()
                
                # Clear the reference
                _enhanced_scanner = None
                log_info("Enhanced scanner cleanup completed")
            except Exception as e:
                log_error(f"Error during scanner cleanup: {e}")
                _enhanced_scanner = None 